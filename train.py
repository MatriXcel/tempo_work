...
def main():
    ...
    for filename in file_names:
        ...
        Ar_slant_col = (np.sum(Ar_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * Ar_amf[0][invalid_matrix]).astype(data_type)
        BrO_slant_col = (np.sum(BrO_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * BrO_amf[0][invalid_matrix]).astype(data_type)
        GLYX_slant_col = (np.sum(GLYX_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * GLYX_amf[0][invalid_matrix]).astype(data_type)
        H2O_slant_col = (np.sum(H2O_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * H2O_amf[0][invalid_matrix]).astype(data_type)
        HCHO_slant_col = (np.sum(HCHO_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * HCHO_amf[0][invalid_matrix]).astype(data_type)
        N2_slant_col = (np.sum(N2_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * N2_amf[0][invalid_matrix]).astype(data_type)
        NO2_slant_col = (np.sum(NO2_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * NO2_amf[0][invalid_matrix]).astype(data_type)
        O2_slant_col = (np.sum(O2_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * O2_amf[0][invalid_matrix]).astype(data_type)
        O3_slant_col = (np.sum(O3_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * O3_amf[0][invalid_matrix]).astype(data_type)
        SO2_slant_col = (np.sum(SO2_gas_mixing_ratio * air_partial_col, axis=0)[invalid_matrix] * SO2_amf[0][invalid_matrix]).astype(data_type)
